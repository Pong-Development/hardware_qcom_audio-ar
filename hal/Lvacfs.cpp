...
void Lvacfs::start_input_stream(StreamInPrimary* in) {
    in->lvacfs_instance = (void**)calloc(1, sizeof(void*));
    if (in->lvacfs_instance == nullptr) {
        ALOGE("Failed to allocate memory for lvacfs_instance");
        return;
    }
    ...
    int ret = wrapper_ops_->create_instance(in->lvacfs_instance, in->source_,
                                            sample_rate_and_format, channels);
    if (ret < 0) {
        ALOGE("create instance failed: %d", ret);
        free(in->lvacfs_instance);
        in->lvacfs_instance = nullptr;
        in->lvacfs_started = false;
    } else {
        in->lvacfs_started = true;
    }
}

void Lvacfs::process_input_stream(StreamInPrimary* in, void* buffer, size_t bytes) {
...
void Lvacfs::stop_input_stream(StreamInPrimary* in) {
    std::lock_guard<std::mutex> lock(in->lvacfs_lock);
    int ret = wrapper_ops_->destroy_instance(in->lvacfs_instance);
    if (ret < 0) {
        ALOGE("destroy_instance failed: %d", ret);
    }
    free(in->lvacfs_instance);
    in->lvacfs_instance = nullptr;
    in->lvacfs_started = false;
